...
const threshold=require('./static/js/threshold.js')
    ...
    req.on('end',async function(){
        ...
        if (items.length>0){
            requestBody["paths"].forEach(element => {
               ...
               if( items[0].tripwire[element.location].length!=0 && (items[0].tripwire[element.location]).includes(element.path)){
                    count+=1;
...
app.post("/sendfile",async (req,res)=>{
    ...
    req.on('end',async function(){
        ...
        if (items[0]["tripwire"]!=undefined && Object.keys(items[0]["tripwire"]).length>1){
